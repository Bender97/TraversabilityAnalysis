...
public:
	...
	int barWidth;
	std::string msg;
	ProgressBar(int barWidth_, std::string &msg_) {barWidth = barWidth_; msg = msg_;}
		   ...
		   SolutionInfo* si, int shrinking)
{
	...
	std::cout << " initializing gradient ..." << std::flush;
	...
	param->train_end_time_secs = std::chrono::steady_clock::now() + std::chrono::seconds(param->secs_to_wait);
...
int Solver::select_working_set(int &out_i, int &out_j)
{
	...
	//std::cout << " eps: " << Gmax+Gmax2 << " / " << eps << std::endl;

	if(Gmax+Gmax2 < eps || Gmin_idx == -1)
		return 1;

	if (true) {
		bool time_to_quit = std::chrono::steady_clock::now() >= param->train_end_time_secs;

		if (time_to_quit) {
			//std::cout << "had to return because it's time to quit!" << std::endl;
			//std::cout << "   eps was " << max(Gmaxp+Gmaxp2,Gmaxn+Gmaxn2) << " / " << eps << std::endl;
			param->final_eps = Gmax+Gmax2;
			return 1;
		}
	}
...
public:
	...
	Solver_NU(svm_parameter *param_) {
		manual_drive=true;
		param = param_;
	}
	Solver_NU() {}
...
void Solver_NU::Solve(int l, const QMatrix& Q, const double *p_, const schar *y_,
		   double *alpha_, double Cp, double Cn, double eps,
		   SolutionInfo* si, int shrinking)
{
	this->si = si;
	...
	// initialize gradient
	{
		std::string msg="Solver initialize gradient";
		ProgressBar pBar(100, msg);
		//std::cout << "Solver initialize gradient\n";
		G = new double[l];
		G_bar = new double[l];
		int i;
		for(i=0;i<l;i++)
		...
		for(i=0;i<l;i++) {
			//std::cout << "i: " << i << " " << !is_lower_bound(i) << std::endl;
			if(!is_lower_bound(i))
			{
				pBar.update((float)i/l);
				const Qfloat *Q_i = Q.get_Q(i,l);
				double alpha_i = alpha[i];
				if (alpha_i<1.0e-8) continue;
				int j;
				//std::cout << "Q_i: ";
				for(j=0;j<l;j++) {
					//std::cout << Q_i[j] << " ";
				...
				//std::cout << std::endl;
				 // G_bar appears to be always zero (for my intents!)
				  if(is_upper_bound(i))
					for(j=0;j<l;j++)
						G_bar[j] += get_C(i) * Q_i[j];
				//std::cout << std::endl;
...
int Solver_NU::select_working_set(int &out_i, int &out_j)
{
	...
	//std::cout << "current eps: " << max(Gmaxp+Gmaxp2,Gmaxn+Gmaxn2) << " / " << eps << std::endl;

	if(max(Gmaxp+Gmaxp2,Gmaxn+Gmaxn2) < eps || Gmin_idx == -1 ) {
		if (manual_drive)
			param->final_eps = max(Gmaxp+Gmaxp2,Gmaxn+Gmaxn2);
		//std::cout << " ended normally " << std::endl;
		return 1;
	}

//	auto end = std::chrono::steady_clock::now();
//        std::chrono::duration<double> diff = train_end_time_secs - end;
//    std::cout << "Time to stop: " << diff.count() << " s\n";
	...
	double *alpha, Solver::SolutionInfo* si)
{
	...
	for(i=0;i<l;i++)
	{
		...
		//std::cout << "min(sum,C): " << alpha2[i] << " " << std::flush;
		sum -= alpha2[i];

		//std::cout << " now_sum: " << sum << " proby: " << prob->y[i] << " " << std::flush;

		linear_term[i] = - prob->y[i];
		//std::cout << "linear_term: " << linear_term[i] << " " << std::endl;
...
double getKernel(const svm_node *x, const svm_node *y, double gamma) {
	double sum = 0;
	while(x->index != -1 && y->index !=-1)
	{
		if(x->index == y->index)
...
double svm_predict_matrix(const svm_model *model, const svm_node *x, double gamma) {
	double *sv_coef = model->sv_coef[0];
	double sum_tot = 0;
	const svm_node *y;
	for(int i=0;i<model->l;i++) {
		sum_tot += sv_coef[i] * getKernel(x, model->SV[i], gamma);
	}
	sum_tot -= model->rho[0];
	//*dec_values = sum;
