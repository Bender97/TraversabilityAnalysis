...
int Feature::computeFeatures(Cell *cell, Eigen::MatrixXd &scene_normal, std::vector<Eigen::Vector3d> &points, float area) {
  cx = 0; cy = 0; cz = 0;
  ...
  for (auto p_idx : cell->points_idx) {
      p = &(points[p_idx]);
      goodness_of_fit +=
              std::abs((nvx*(*p)(0) + nvy*(*p)(1) + nvz*(*p)(2)) + d) / normal_magnitude;
